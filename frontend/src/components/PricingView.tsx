...
export default function PricingView({ onAuthRequest }: PricingViewProps) {
    ...
    const premiumPrice = billingCycle === 'annual' ? 6.20 : 7.33;
            ...
            <div className="max-w-6xl mx-auto w-full px-6 grid md:grid-cols-3 gap-6 lg:gap-8 mb-16">
                ...
                <div className="bg-white dark:bg-slate-800/40 border border-gray-200 dark:border-slate-700/50 rounded-3xl p-8 flex flex-col hover:bg-gray-50 dark:hover:bg-slate-800/60 transition-all shadow-sm">
                    ...
                    <button
                        ...
                        className="w-full py-3.5 bg-white dark:bg-slate-800 border-2 border-emerald-600 dark:border-emerald-500 text-emerald-600 dark:text-emerald-400 rounded-full font-bold hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all"
                    >
                        Sign up
                    </button>
                </div>
                ...
                {/* Premium Plan */}
                <div className="bg-white dark:bg-slate-800/40 border-2 border-emerald-600 dark:border-emerald-500 rounded-3xl p-8 flex flex-col relative shadow-xl shadow-emerald-500/10">
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-emerald-600 dark:bg-emerald-500 text-white px-4 py-1 rounded-full text-sm font-bold flex items-center gap-1.5 whitespace-nowrap">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                        Premium
                    </div>
                    <h3 className="text-xl font-bold mb-4 text-emerald-700 dark:text-emerald-400 text-center mt-2">Premium</h3>
                    <div className="flex flex-col items-center justify-center mb-6 h-24">
                        <div className="flex items-baseline gap-1">
                    ...
                    <button
                        onClick={onAuthRequest}
                        className="w-full py-3.5 bg-emerald-600 dark:bg-emerald-500 text-white rounded-full font-bold hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-all shadow-md"
                    >
                        Upgrade
                    </button>
                </div>
                            ...
                            <table className="w-full text-left border-collapse min-w-[600px]">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="p-6 border-b border-gray-100 dark:border-slate-700/50 text-center font-bold text-emerald-600 dark:text-emerald-400 w-1/5">Premium</th>
                                ...
                                <tbody>
                                    <tr className="bg-gray-50/50 dark:bg-white/[0.02]">
                                        <td colSpan={4} className="p-4 font-black text-slate-900 dark:text-emerald-400">Rewriting</td>
                                    </tr>
                                    ...
                                    <tr className="bg-gray-50/50 dark:bg-white/[0.02]">
                                        <td colSpan={4} className="p-4 font-black text-slate-900 dark:text-emerald-400 mt-4">Refining & Detection</td>
                                    </tr>
                                    ...
                                    <tr className="bg-gray-50/50 dark:bg-white/[0.02]">
                                        <td colSpan={4} className="p-4 font-black text-slate-900 dark:text-emerald-400 mt-4">Customer Support</td>
                                    </tr>
...
function TableRow({ feature, free, premium, student, isBg = false }: { feature: string, free: React.ReactNode, premium: React.ReactNode, student: React.ReactNode, isBg?: boolean }) {
    const renderCell = (value: React.ReactNode) => {
        if (value === true) {
            return <svg className="w-5 h-5 text-emerald-500 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>;
        }
        if (value === false) {
    ...
    return (
        <tr className={`border-b border-gray-50 dark:border-slate-700/30 hover:bg-gray-50/80 dark:hover:bg-slate-700/20 transition-colors ${isBg ? 'bg-gray-50/30 dark:bg-slate-800/20' : ''}`}>
            <td className="p-4 pl-6 text-sm font-medium text-slate-700 dark:text-gray-300 flex items-center gap-3">
                <div className="w-4 h-4 bg-emerald-100 dark:bg-emerald-500/20 rounded flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0">
