...
export default function AuthForm({ onPrivacyClick, onTermsClick }: AuthFormProps = {}) {
    ...
    if (CLERK_KEY) {
        ...
        return (
            <div className="w-full max-w-md mx-auto animate-fade-in-up">
                ...
                {mode === 'login' ? (
                    <SignIn appearance={clerkAppearance} routing="hash" forceRedirectUrl="/" />
                ) : (
                    <SignUp appearance={clerkAppearance} routing="hash" forceRedirectUrl="/" />
                )}
    ...
    // ─── Legacy email/password form (fallback when no Clerk key) ───
            ...
            if (mode === 'login') {
                try { await login(email, password); } catch (err: any) {
                    if (err.message === 'ACCOUNT_HAS_NO_PASSWORD') {
                        setMode('set-password'); clearError(); setPassword(''); setConfirmPassword('');
                    }
                }
            } else if (mode === 'signup') {
            ...
            <div className="bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-gray-100 dark:border-slate-800 rounded-[3rem] p-10 shadow-2xl transition-colors">
                ...
                {(mode === 'login' || mode === 'signup') && (
                    <div className="flex bg-gray-100 dark:bg-slate-800/50 rounded-[1.5rem] p-1.5 mb-10">
                        <button type="button" onClick={() => switchMode('login')}
                            className={`flex-1 py-3 rounded-[1.2rem] font-black text-sm transition-all duration-300 ${mode === 'login' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-white shadow-xl' : 'text-slate-400 dark:text-gray-500'}`}>Login</button>
                        <button type="button" onClick={() => switchMode('signup')}
                            className={`flex-1 py-3 rounded-[1.2rem] font-black text-sm transition-all duration-300 ${mode === 'signup' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-white shadow-xl' : 'text-slate-400 dark:text-gray-500'}`}>Sign Up</button>
                    </div>
                )}
                {error && (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    {mode === 'signup' && (
                        <div className="space-y-2">
                            ...
                            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Jane Doe"
                                className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 text-slate-900 dark:text-white font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/10 placeholder:text-slate-300" />
                        </div>
                    )}
                    <div className="space-y-2">
                        ...
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)} disabled={mode === 'set-password'} placeholder="user@example.com"
                            className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 text-slate-900 dark:text-white font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/10 disabled:opacity-75 placeholder:text-slate-300" />
                    </div>
                    <div className="space-y-2">
                        <div className="flex justify-between items-center px-1">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Password</label>
                            {mode === 'login' && <button type="button" className="text-[10px] uppercase font-black text-blue-600 tracking-widest">Forgot?</button>}
                        </div>
                        <div className="relative">
                            <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••"
                                className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 text-slate-900 dark:text-white font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/10 placeholder:text-slate-300" />
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 w-8 h-8 rounded-full hover:bg-slate-100 transition-colors">
                    ...
                    {(mode === 'signup' || mode === 'set-password') && (
                        <div className="space-y-2">
                            <label className="text-xs font-black text-slate-400 ml-1 uppercase tracking-widest">Confirm Password</label>
                            <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="••••••••"
                                className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 text-slate-900 dark:text-white font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/10 placeholder:text-slate-300" />
                        </div>
                    )}
                    <button type="submit" disabled={isSubmitting || isLoading}
                        className="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-[1.5rem] font-black text-sm tracking-widest uppercase shadow-2xl shadow-blue-500/40 hover:scale-[1.02] active:scale-95 disabled:opacity-50 transition-all duration-300">
                        {isSubmitting ? 'Processing...' : mode === 'login' ? 'LOGIN' : mode === 'signup' ? 'CREATE ACCOUNT' : 'SET PASSWORD & LOGIN'}
                    </button>
                </form>
