...
async def get_admin_metrics(
    ...
    detection_metrics = DetectionMetrics(
        ...
        avg_latency_ms=statistics.mean(detection_latencies)
...
async def get_detailed_health(
    ...
    recent_requests = [
        r for r in _metrics_db["requests"] if r["timestamp"] > five_min_ago
    ]
    recent_errors = [r for r in recent_requests if r["status_code"] >= 400]
    recent_error_rate = (
        (len(recent_errors) / len(recent_requests) * 100) if recent_requests else 0.0
    )
