...
class DetectRequest(BaseModel):
    text: str = Field(
        ..., min_length=1, max_length=20000, description="The text to analyze"
    )
...
class DetectResponse(BaseModel):
    ...
    probability: float = Field(
        ..., ge=0.0, le=1.0, description="The confidence probability"
    )
...
class ParaphraseRequest(BaseModel):
    ...
    text: str = Field(
        ..., min_length=1, max_length=10000, description="The text to paraphrase"
    )
    mode: ParaphraseMode = Field(
        default="standard", description="Paraphrasing style mode"
    )
    temperature: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Controls randomness (0.0-1.0)"
    )
    max_length: int = Field(
        default=512, ge=50, le=1024, description="Maximum output length in tokens"
    )
